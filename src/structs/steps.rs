use iced::{
  ...
  widget::{ 
    Column, Container, 
    slider, checkbox, column, text, radio, row, toggler, text_input, vertical_space 
  }, 
  Length, Color, Element, Font
};

use crate::{
  enums::{Step, Layout, StepMessage, Language}, 
  components::{color_slider, ferris}
};
...
impl Steps {
  ...
  pub fn view(&self, debug: bool) -> Element<StepMessage> {
...
impl<'a> Step {
  ...
  pub fn slider(value: u8) -> Column<'a, StepMessage> {
  ...
  pub fn text(size: u16, color: Color) -> Column<'a, StepMessage> {
  ...
  pub fn toggler(can_continue: bool) -> Column<'a, StepMessage> {
  ...
  pub fn image(width: u16) -> Column<'a, StepMessage> {
